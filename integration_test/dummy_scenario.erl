...
%% var1 must be equal to one of the values in the list and default value is def1
-required_variable({var1, "description1", def1, [def1, another_value]}).

-required_variable([
    %% var2 must be positively verified by test_verification_function/1 function.
    %% this function must be exported from the scenario module, or from any of
    %% the modules specified in config_verification_modules amoc configuration
    %% variable.
    {var2, "description2", def2, test_verification_function},
    %% alternatively verification function can be supplied as function pointer
    %% in 'fun module:function/arity' format. note that it must be exported function
...
%% none is predefined verification function which accepts all the values
...
-spec init() -> ok.
init() ->
    %% amoc follows a couple of rules during scenario initialisation:
    %%  - if any parameter verification fails, amoc will not start
    %%    the scenario and init/0 function is not triggered.
    %%  - if init/0 function fails, amoc will not start any user (by
    %%    calling start/1 or start2 function)
    %% if REST API reports that scenario is executed, than all the
    ...
    %% it doesn't matter if undeclared_variable is passed through the
